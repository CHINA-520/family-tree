export default {
  async fetch(request, env) {
    ...
    <head>
      ...
      <title>莫氏族谱-生卒纪年版</title>
      <script src="https://d3js.org/d3.v7.min.js"></script>
      <style>
        :root { --m-red: #9d2933; --m-bg: #f5f2e9; --m-orange: #e67e22; }
        body { background: var(--m-bg); font-family: "Microsoft YaHei", sans-serif; margin: 0; padding-bottom: 20px; }
        .header { text-align: center; padding: 20px; background: var(--m-bg); border-bottom: 3px solid var(--m-red); position: sticky; top:0; z-index:100; }
        .nav { display: flex; flex-wrap: wrap; justify-content: center; background: #e0e0e0; padding: 5px; border-radius: 10px; margin-bottom: 10px; gap: 4px; }
        ...
        @media print { .header, .nav, button, .stats-bar, .footer, #panel { display: none !important; } body { background: white; } }
      </style>
    </head>
    <body>
      <div id="lock-screen" style="position:fixed; inset:0; background:var(--m-bg); z-index:9999; display:flex; align-items:center; justify-content:center;">
        <div style="background:#fff; padding:35px; border-radius:15px; text-align:center; width:280px;">
          <h2 style="color:var(--m-red);">莫氏族谱</h2>
      ...
      <div id="main-app" style="display:none;">
        <header class="header">
          <h1 style="color:var(--m-red); margin:0 0 15px 0;">莫氏族谱</h1>
          <div class="nav">
      ...
      <script>
        ...
        function unlock() { if(document.getElementById('pw').value === VIEW_PW) { document.getElementById('lock-screen').style.display = 'none'; document.getElementById('main-app').style.display = 'block'; renderStats(); tab('list'); } else alert('密码错误'); }
        ...
        function renderList() {
          ...
          const groups = {}; data.forEach(m => { const g = m.gen || '待补充'; if(!groups[g]) groups[g] = []; groups[g].push(m); });
          Object.keys(groups).sort((a,b)=>parseInt(a.match(/\\d+/))-parseInt(b.match(/\\d+/))).forEach(g => {
            ...
            groups[g].forEach(m => {
              ...
              h += \`<div class="card" onclick="openPanel('\${m.id}')">
                <div class="name-area">莫 · \${m.name} <span class="life-span">\${span}</span></div>
