export default {
  async fetch(request, env) {
    ...
    const SYS_CONF = {
      familyName: "莫",             // 姓氏
      versionName: "生卒纪年版",      // 版本/支系名称
      orgName: "莫氏宗亲会",         // 底部署名机构
      themeColor: "#9d2933",       // 主题色（默认故宫红）
      viewPassword: "666",         // 访问密码
      editPassword: "888",         // 管理/编辑密码
    ...
    if (request.method === 'POST') {
      try {
        ...
        if (body.auth !== SYS_CONF.editPassword) return new Response('权限不足', { status: 403 });
    ...
    <head>
      ...
      <style>
        ...
        body { background: var(--m-bg); font-family: "Microsoft YaHei", sans-serif; margin: 0; padding-bottom: 20px; color: #333; }
        ...
        .search-bar { width: 100%; max-width: 500px; margin: 0 auto 10px auto; position: relative; }
        .search-bar input { width: 100%; padding: 10px 15px; border: 1.5px solid #ddd; border-radius: 25px; box-sizing: border-box; outline: none; transition: 0.3s; }
        .search-bar input:focus { border-color: var(--m-red); box-shadow: 0 0 8px rgba(157,41,51,0.1); }
        .container { max-width: 1200px; margin: 0 auto; padding: 15px; min-height: 60vh; }
        .card { background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #eee; cursor: pointer; margin-bottom: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); transition: 0.2s; }
        ...
        .lbl { color: #999; width: 95px; flex-shrink: 0; font-weight: bold; }
        ...
        @media print { .header, button, .stats-bar, .footer, #panel, .search-bar { display: none !important; } }
      </style>
    </head>
    <body>
      <div id="lock-screen" style="position:fixed; inset:0; background:var(--m-bg); z-index:9999; display:flex; align-items:center; justify-content:center;">
        <div style="background:#fff; padding:35px; border-radius:15px; text-align:center; width:280px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
          <h2 style="color:var(--m-red); margin-top:0;">${SYS_CONF.familyName}氏族谱</h2>
          <input type="password" id="pw" placeholder="输入查看密码" style="text-align:center;">
      ...
      <div id="main-app" style="display:none;">
        <header class="header">
          <h1 style="color:var(--m-red); margin:0 0 5px 0;">${SYS_CONF.familyName}氏族谱</h1>
          <div class="search-bar">
            <input type="text" id="search-input" placeholder="🔍 搜索姓名、地点、简介..." oninput="doSearch()">
          </div>
          <div class="nav">
        ...
        <div id="view-table" class="container" style="display:none;">
            <div style="background:#fff; border-radius:12px; overflow-x:auto;">
              <table style="width:100%; border-collapse:collapse; min-width:800px;">
                <thead><tr style="background:#f8f8f8;"><th style="padding:12px;">代别</th><th>姓名</th><th>生卒年</th><th>排行</th><th>配偶</th><th>居住地</th></tr></thead>
        ...
        <div id="view-export" class="container" style="display:none;">
            <div style="text-align:center; padding:50px; background:#fff; border-radius:15px;">
              <h3>📂 族谱数据管理</h3>
              <button class="btn-save" onclick="exportToExcel()" style="width:auto; padding:12px 25px;">导出 Excel (.csv)</button>
              <button class="btn-save" onclick="exportToJSON()" style="width:auto; padding:12px 25px; background:#666; margin-left:10px;">导出 JSON 备份</button>
              <p style="color:#999; font-size:12px; margin-top:20px;">定期备份数据是传承的重要保障</p>
            </div>
        </div>

        <div class="stats-bar">
          <div style="font-weight:bold; color:var(--m-red); border-left:4px solid var(--m-red); padding-left:10px;">家族统计</div>
        ...
        <div id="panel">
          <h3 style="border-left:4px solid var(--m-red); padding-left:10px;">成员档案编辑</h3>
          ...
          <div style="margin-bottom:12px;"><label>排行顺序</label><input id="in-order" placeholder="如：长子、次女"></div>
          <div style="margin-bottom:12px;"><label>所属代别</label><input id="in-gen" placeholder="如：第5代"></div>
          <div style="margin-bottom:12px;"><label>配偶信息</label><input id="in-spouse"></div>
          <div style="margin-bottom:12px;"><label>居住地址</label><input id="in-loc"></div>
          <div style="margin-bottom:12px;"><label>个人简介</label><textarea id="in-bio" rows="4"></textarea></div>
          <button class="btn-save" onclick="doSave()">💾 保存修改</button>
          ...
          <div style="margin-top:20px; text-align:center;">
            <button style="color:red; background:none; border:none; cursor:pointer;" onclick="doDel()">❌ 永久删除</button>
            <span style="color:#eee; margin:0 10px;">|</span>
            <button style="color:#999; background:none; border:none; cursor:pointer;" onclick="closePanel()">返回</button>
          </div>
        </div>
      </div>

      <script>
        const CONF = { familyName: "${SYS_CONF.familyName}", viewPw: "${SYS_CONF.viewPassword}" };
        let data = ${rawData};

        function unlock() { 
          if(document.getElementById('pw').value === CONF.viewPw) { 
        ...
        function doSearch() {
          ...
          const filtered = data.filter(m => 
            ...
            (m.loc && m.loc.toLowerCase().includes(kw)) ||
        ...
        function renderList(targetData = data) {
          ...
          if(targetData.length === 0) { list.innerHTML = '<p style="text-align:center;color:#999;margin-top:50px;">未找到匹配成员</p>'; return; }
        ...
        function renderTree() {
          ...
          function build(n) {
            ...
            d.innerHTML = \`<div onclick="openPanel('\${n.id}')" style="cursor:pointer; padding:8px 0;">
                <strong style="font-size:16px;">\${n.name}</strong> <small style="color:#999;">(\${n.birth||''}-\${n.death||''})</small> 
                <span style="color:var(--m-orange); font-size:13px; margin-left:10px;">[配偶：\${n.spouse || '无'}]</span>
              </div>\`;
        ...
        function addNew() { const n = prompt("请输入家族始祖(第一代)姓名："); if(n){ data.push({id:"id"+Date.now(), name:n, gen:"第1代"}); apiPost(data); } }
        function addSon() {
          const n = prompt("请输入后代姓名："); if(n){
            const p = data.find(f => f.id === window.cur);
            const nextGen = "第" + ((parseInt(p.gen.match(/\\d+/)) || 0) + 1) + "代";
            data.push({id:"id"+Date.now(), name:n, gen:nextGen, pid:p.id});
            apiPost(data);
          }
        }
        function doDel() { if(confirm("警告：确定要永久删除该成员及其所有档案吗？")){ data=data.filter(f=>f.id!==window.cur); apiPost(data); } }
        ...
        function exportToExcel() {
          let csv = "\\ufeff代别,姓名,出生年份,卒年,配偶,排行,居住地,简介\\n";
          data.forEach(m => { csv += \`"\${m.gen}","\${m.name}","\${m.birth||''}","\${m.death||''}","\${m.spouse||''}","\${m.order||''}","\${m.loc||''}","\${(m.bio||'').replace(/"/g,'""')}"\\n\`; });
          downloadFile(csv, CONF.familyName + '氏族谱档案.csv', 'text/csv;charset=utf-8;');
        }
        function exportToJSON() { downloadFile(JSON.stringify(data, null, 2), CONF.familyName + '氏族谱备份.json', 'application/json'); }
